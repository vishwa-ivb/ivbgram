...
function messagepop() {
  document.getElementById("msgpop").submit();
  //chatarea.scrollTop = chatarea.scrollHeight;
}
var totalmsgs
function colordif(){
	var sender = $("#sender").val();
	totalmsgs = $("#totalmsgs").val()+1;
	console.log(sender);
	console.log(totalmsgs);
	for(var i=1;i<=totalmsgs;i++){
		var sendernow = $("#eachsender"+i).val();
		if(sender == sendernow){
			$("#eachmsg"+i).css('background-color','rgba(0,0,0,.5)');
			$("#eachmsg"+i).css('color','#03e9f4');
			$("#foralign"+i).css('text-align','right');
		}
		else{
			$("#eachmsg"+i).css('background-color','#03e9f4');
			$("#eachmsg"+i).css('color','rgba(0,0,0,.5)');
			$("#foralign"+i).css('text-align','left');
		}
	}
}
...
function send(){
	...
	messagedata.append('sender', sender);
    ...
    contentType: false,
    cache: false,
    success: function(messagedata, statusText, xhr) {
      console.log(xhr.status);
      if (xhr.status == "200") {
	//$("#chatarea").load(" #chatarea > *",{sender,receiver});
	//$('#yourDiv').load('#yourDiv > *')
	 	$( "#gotob" ).load(window.location.href + " #gotob",{sender,receiver} );
...
function updateDiv()
{ 
    //$( "#chatarea" ).load(window.location.href + " #chatarea" );
    //$("#chatarea").load(" #chatarea > *");
    setInterval(tmr, 1000);
	var val = 0;
	
	function tmr() {
	    
	    chatarea.load;//innerText = ++val;
	}
}